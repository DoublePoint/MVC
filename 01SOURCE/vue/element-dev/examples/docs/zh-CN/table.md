...
:::demo 当`ll-table`元素中注入`data`对象数组后，在`ll-table-column`中用`prop`属性来对应对象中的键名即可填入数据，用`label`属性来定义表格的列名。可以使用`width`属性来定义列宽。
```html
  <template>
    <ll-table
      :data="tableData"
...
:::demo 只要在`ll-table`元素中定义了`height`属性，即可实现固定表头的表格，而不需要额外的代码。
```html
<template>
  <ll-table
    :data="tableData3"
  ...
  <ll-table
    ...
    <ll-table-column
      ...
      <template slot-scope="scope">
        <ll-button @click="handleClick(scope.row)" type="text" size="small">查看</ll-button>
        <ll-button type="text" size="small">编辑</ll-button>
      </template>
    </ll-table-column>
  </ll-table>
</template>
...
:::demo 只需要在 ll-table-column 里面嵌套 ll-table-column，就可以实现多级表头。
```html
<template>
  <ll-table
    :data="tableData3"
    ...
    <ll-table-column label="配送信息">
      <ll-table-column
        prop="name"
        label="姓名"
        width="120">
      </ll-table-column>
      <ll-table-column label="地址">
        <ll-table-column
          prop="province"
          label="省份"
          width="120">
...
:::demo Table 组件提供了单选的支持，只需要配置`highlight-current-row`属性即可实现单选。之后由`current-change`事件来管理选中时触发的事件，它会传入`currentRow`，`oldCurrentRow`。如果需要显示索引，可以增加一列`ll-table-column`，设置`type`属性为`index`即可显示从 1 开始的索引号。
```html
<template>
  <ll-table
    ref="singleTable"
  ...
  <div style="margin-top: 20px">
    <ll-button @click="setCurrent(tableData[1])">选中第二行</ll-button>
    <ll-button @click="setCurrent()">取消选择</ll-button>
  </div>
</template>
...
:::demo 实现多选非常简单: 手动添加一个`ll-table-column`，设`type`属性为`selection`即可；默认情况下若内容过多会折行显示，若需要单行显示可以使用`show-overflow-tooltip`属性，它接受一个`Boolean`，为`true`时多余的内容会在 hover 时以 tooltip 的形式显示出来。
```html
<template>
  <ll-table
    ref="multipleTable"
  ...
  <div style="margin-top: 20px">
    <ll-button @click="toggleSelection([tableData3[1], tableData3[2]])">切换第二、第三行的选中状态</ll-button>
    <ll-button @click="toggleSelection()">取消选择</ll-button>
  </div>
</template>
...
```html
<template>
  <ll-button @click="resetDateFilter">清除日期过滤器</ll-button>
  <ll-button @click="clearFilter">清除所有过滤器</ll-button>
  <ll-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
    ...
    <ll-table-column
      ...
      <template slot-scope="scope">
        <ll-tag
          ...
          disable-transitions>{{scope.row.tag}}</ll-tag>
      </template>
    </ll-table-column>
  </ll-table>
</template>
...
<template>
  <ll-table
    ...
    <ll-table-column
      ...
      width="180">
      <template slot-scope="scope">
        <ll-popover trigger="hover" placement="top">
          <p>姓名: {{ scope.row.name }}</p>
          <p>住址: {{ scope.row.address }}</p>
          <div slot="reference" class="name-wrapper">
            <ll-tag size="medium">{{ scope.row.name }}</ll-tag>
          </div>
        </ll-popover>
      </template>
    </ll-table-column>
    <ll-table-column label="操作">
      <template slot-scope="scope">
        <ll-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">编辑</ll-button>
        <ll-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">删除</ll-button>
      </template>
    </ll-table-column>
  </ll-table>
</template>
...
:::demo 通过设置 type="expand" 和 `Scoped slot` 可以开启展开行功能，`ll-table-column` 的模板会被渲染成为展开行的内容，展开行可访问的属性与使用自定义列模板时的 `Scoped slot` 相同。
```html
<template>
  <ll-table
    :data="tableData5"
    ...
    <ll-table-column type="expand">
      <template slot-scope="props">
        <ll-form label-position="left" inline class="demo-table-expand">
          <ll-form-item label="商品名称">
            <span>{{ props.row.name }}</span>
          </ll-form-item>
          <ll-form-item label="所属店铺">
            <span>{{ props.row.shop }}</span>
          </ll-form-item>
          <ll-form-item label="商品 ID">
            <span>{{ props.row.id }}</span>
          </ll-form-item>
          <ll-form-item label="店铺 ID">
            <span>{{ props.row.shopId }}</span>
          </ll-form-item>
          <ll-form-item label="商品分类">
            <span>{{ props.row.category }}</span>
          </ll-form-item>
          <ll-form-item label="店铺地址">
            <span>{{ props.row.address }}</span>
          </ll-form-item>
          <ll-form-item label="商品描述">
  ...
  <ll-table
    ...
    <ll-table-column
      ...
      <template slot-scope="scope">
        <ll-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Edit</ll-button>
        <ll-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</ll-button>
      </template>
    </ll-table-column>
  </ll-table>
</template>
...
| resizable | 对应列是否可以通过拖动改变宽度（需要在 ll-table 上设置 border 属性为真） | boolean | — | true |
