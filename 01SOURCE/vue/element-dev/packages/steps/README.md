...
```javascript
import Vue from 'vue'
import LlStep from 'element-steps'
...
```javascript
import Vue from 'vue'
import { LlSteps, LlStep } from 'element-steps'

Vue.component('el-steps', LlSteps)
Vue.component('el-step', LlStep)
```
...
| icon | 图标 | Llement Icon 提供的图标，如果要使用自定义图标可以通过 slot 方式写入 | string | — |
