(function($) {
	var componentTemplate = '<div class="layui-inline" >' + '<label class="layui-form-label" :style="labelclientStyle">{{requiredIconText}}{{"&nbsp;&nbsp;"+title+"："}}</label>'
			...
			+ '<input :id="inputButtonHiddenId" type="hidden" value="" :name="field" />' + '<a :id="inputButtonButtonId" href="#" class="layui-btn inputbutton">...</a>' + '</div>'
			+ '</div>';
	_LL_Model.formFieldComponent(_LL_Constant._ConstantComponentMap._FormInputButton, componentTemplate);

	_LL_Model.FormInputButton = function(domId) {
	...
	// 将实例作为子类的原型
	_LL_Model.FormInputButton.prototype = new Super();
	_LL_Model.FormInputButton.prototype.constructor = _LL_Model.FormInputButton; // 需要修复下构造函数
})(jQuery);
